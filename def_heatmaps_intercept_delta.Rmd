---
title: "Heatmaps for Varying Intercepts and Deltas"
output:
...
n_sim <- 1000  
intercept_values <- seq(1, 10, length.out = 10)
true_slope <- 1
```
...
run_simulation <- function(n_sim, n, intercept_values, true_slope, x_m, alpha, delta_values) {
  # Initialize results storage
  results <- expand.grid(
    true_delta = delta_values,
    true_intercept = intercept_values,
    mse_mod_bregman = NA,
  ...
  for (i in seq_len(nrow(results))) {
    ...
    true_intercept <- results$true_intercept[i]
    ...
    for (j in 1:n_sim) {
      ...
      mse_mod_bregman[j] <- (bregman$coef[1] - true_intercept)^2
      mse_mod_bregman_huber[j] <- (bregman_huber$coef[1] - true_intercept)^2
      mad_mod_bregman[j] <- abs(bregman$coef[1] - true_intercept)
      mad_mod_bregman_huber[j] <- abs(bregman_huber$coef[1] - true_intercept)
      mse_mod_ols[j] <- (ols$coef[1] - true_intercept)^2
      mad_mod_ols[j] <- abs(ols$coef[1] - true_intercept)
    }
    
    results$mse_mod_bregman[i] <- 
      sqrt(median(mse_mod_bregman, na.rm = TRUE)) / true_intercept
    results$mse_mod_bregman_huber[i] <- 
      sqrt(median(mse_mod_bregman_huber, na.rm = TRUE)) / true_intercept
    results$mad_mod_bregman[i] <- 
      median(mad_mod_bregman, na.rm = TRUE) / true_intercept
    results$mad_mod_bregman_huber[i] <- 
      median(mad_mod_bregman_huber, na.rm = TRUE) / true_intercept
    results$mse_mod_ols[i] <- 
      sqrt(median(mse_mod_ols, na.rm = TRUE)) / true_intercept
    results$mad_mod_ols[i] <- 
      median(mad_mod_ols, na.rm = TRUE) / true_intercept
...
sim_results <- run_simulation(n_sim, n, intercept_values, true_slope, x_m, alpha, delta_values)
```
...
library(ggplot2)
ggplot(sim_results, aes(x = true_intercept, y = true_delta, fill = mse_mod_bregman)) +
  geom_tile() +
...
ggplot(sim_results, aes(x = true_intercept, y = true_delta, fill = mse_mod_bregman_huber)) +
  geom_tile() +
  scale_fill_gradientn(
    colors = c("blue", "white", "red"),
    name = "Modified MSE"
...
library(ggplot2)
ggplot(sim_results, aes(x = true_intercept, y = true_delta, fill = mad_mod_bregman)) +
...
ggplot(sim_results, aes(x = true_intercept, y = true_delta, fill = mad_mod_bregman_huber)) +
  geom_tile() +
  scale_fill_gradientn(
    colors = c("blue", "white", "red"),
    name = "Modified MAD"
